...
{/* ---Left Team--- */}
  <div className="h-[50vh] sm:h-screen p-4 rounded grid sm:grid-rows-[1fr_0.5fr_1.5fr_1.5fr_1.5fr_0.5fr_1fr_0.5fr] sm:grid-cols-1">
    ...
    <div className="grid grid-cols-4 row-start-7">
      <div className="bg-gradient-to-r from-[#BA94D1] to-[#7F669D] rounded-xl text-center flex justify-center items-center text-16 md:text-3xl text-white cursor-pointer border-0" data-name="subtScoreA" onClick={count}><i className="ri-subtract-line"></i></div>
      <div className="bg-gradient-to-r from-[#BA94D1] to-[#7F669D] rounded-xl text-center flex justify-center items-center text-16 md:text-3xl text-white cursor-pointer border-0 col-start-4" data-name="addScoreA" onClick={count}><i className="ri-add-line"></i></div>
    </div>
  </div>
{/* End Left Team */}
...
{/* ---WinSet Point--- */}
    <div className="text-4xl text-white col-start-2 place-self-center">{winSetA}</div>
...
<span className="text-[8px] block">Baru</span>
</div>
...
{/* End Center Panel */}
  ...
  <div className="backdrop-blur-lg bg-[#ffffff4d] w-full border-4 border-[#ffffff4d] text-8 text-center text-white font-bold rounded-xl flex items-center justify-evenly cursor-pointer">
    ...
    <div className="grid grid-cols-4 row-start-7">
      <div className="bg-gradient-to-r from-[#FCDDB0] to-[#FF9F9F] rounded-xl text-center text-16 md:text-3xl flex justify-center items-center text-white cursor-pointer border-0" data-name="subtScoreB" onClick={count}><i className="ri-subtract-line"></i></div>
      <div className="bg-gradient-to-r from-[#FCDDB0] to-[#FF9F9F] rounded-xl text-center text-16 md:text-3xl flex justify-center items-center text-white cursor-pointer border-0 col-start-4" data-name="addScoreB" onClick={count}><i className="ri-add-line"></i></div>
    </div>
  </div>
  {/* End Right Team */}
...
<div className="bg-gradient-to-r to-[#9ED5C5] from-[#94a3b8] top-0 bottom-0 right-0 left-0 absolute z-20 grid grid-cols-1 sm:grid-cols-[1fr_1.5fr_1fr] h-full sm:h-screen sm:w-screen sm:gap-x-[20px] sm:gap-y-[10px] gap-[12px] animate-small-large">
    ...
    {/* ---Close Button---*/}    
  <div className="absolute h-10 w-10 rounded-full bg-[#fb7185] text-[2em] text-white top-[1%] right-[40%] col-start-2 cursor-pointer z-50 flex justify-center items-center opacity-75" onClick={()=> setListScores(false)}><i className="ri-close-line opacity-100"></i>
  </div>
  {/* End Close Button */}
...
{!localScores == false ?  
  ...
  <div className="grid grid-cols-[1fr_1fr_10px_1fr_1fr] grid-rows-[1fr_0.5fr_1fr_24px] h-[50%] py-8" keys={score.id}>
  <i className="ri-user-6-fill place-self-center text-[2em] text-[#7F669D]"></i>
  <span className="justify-self-center text-white text-3xl">{score.data[Object.keys(score.data).length-1].winset.a}</span>
  <span className="col-start-1 row-start-2 justify-self-center text-white">{score.data[0].name.a}</span>
<span className="col-start-3 justify-self-center text-white text-3xl">:</span>
  <i className="ri-user-6-fill col-start-5 place-self-center text-[2em] text-[#fb7185]"></i>
  <span className="col-start-4 row-start-1 justify-self-center text-white text-3xl">{score.data[Object.keys(score.data).length-1].winset.b}</span>
  <span className="col-start-5 justify-self-center text-white">{score.data[0].name.b}</span>
  <div className="bg-gradient-to-r to-[#E97777] from-[#FF9F9F] rounded-xl w-[50%] h-[60%] text-center text-lg flex items-center justify-center text-white cursor-pointer max-sm:text-xl border-0 col-start-1 col-span-5 row-start-3 place-self-center" data-id={score.id} onClick={deleteScore}>HAPUS</div>

      {/* Line */}
      <span className="bg-[#ffffff4d] h-[4px] w-[90%] rounded-xl row-start-5 col-start-1 col-span-5 place-self-center"></span>
{/* End Line */}
...
{notifFailedScores && <div className="w-fit h-fit p-4 absolute bg-[#64748b] text-white font-bold rounded-lg left-[50%] top-[10%] translate-y-[-50%] translate-x-[-50%]">Simpan Skor Gagal
    ...
    {/* ---Close Button---*/}    
    <div className="absolute h-5 w-5 rounded-full bg-[#fb7185] text-white top-[-10%] right-[-5%] col-start-2 cursor-pointer z-50 flex justify-center items-center opacity-75" onClick={()=> setNotifFailedScores(false)}><i className="ri-close-line opacity-100"></i>
  </div>
  {/* End Close Button */}
</div>}
...
{notifSaveScores && <div className="w-fit h-fit p-4 absolute bg-[#34d399] text-white font-bold rounded-lg left-[50%] top-[10%] translate-y-[-50%] translate-x-[-50%]">Simpan Skor Berhasil
    ...
    {/* ---Close Button---*/}    
    <div className="absolute h-5 w-5 rounded-full bg-[#fb7185] text-white top-[-10%] right-[-5%] col-start-2 cursor-pointer z-50 flex justify-center items-center opacity-75" onClick={()=> setNotifSaveScores(false)}><i className="ri-close-line opacity-100"></i>
  </div>
  {/* End Close Button */}
</div>}
{/* End Notif Failed Save Scores */}
...
{notifDeleteScore && <div className="w-fit h-fit p-4 absolute bg-[#e11d48] text-white font-bold rounded-lg left-[50%] top-[10%] translate-y-[-50%] translate-x-[-50%] z-[100]">Hapus data berhasil
    ...
    {/* ---Close Button---*/}    
    <div className="absolute h-5 w-5 rounded-full bg-[#fb7185] text-white top-[-10%] right-[-5%] col-start-2 cursor-pointer z-50 flex justify-center items-center opacity-75" onClick={()=> setNotifDeleteScore(false)}><i className="ri-close-line opacity-100"></i>
  </div>
  {/* End Close Button */}
</div>}
{/* End Notif Failed Save Scores */}
